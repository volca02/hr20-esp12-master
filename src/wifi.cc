...
void ICACHE_FLASH_ATTR resetWifi() {
...
void ICACHE_FLASH_ATTR setupWifi(Config &config, bool cfgLoaded) {
    ...
    WiFiManagerParameter rfm_pass(
        "rfm_pass", "RFM pass", config.rfm_pass_hex, 16,
    ...
    if (saveConfig)
    {
        strncpy(config.rfm_pass_hex, rfm_pass.getValue(), sizeof(config.rfm_pass_hex));
        ...
        strncpy(config.mqtt_port, mqtt_port.getValue(),
